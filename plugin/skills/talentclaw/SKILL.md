...
The plugin gives Claude the TalentClaw career advisor skill with career intelligence and workflow patterns.
...
6. *First search* — search for opportunities, walk through top results with genuine assessments, help apply to the best match if there is one.
...
| Receive message | Creates `messages/{thread}/{timestamp}.md` files |
...
- Application notes should be concise and targeted -- see the Application Playbook.
